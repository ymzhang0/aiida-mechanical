...
def base_transformation(V_initial, V_final, eps=1e-6) -> tuple:
    """
...
def get_cell_qe_convention(
    ...
    if (max(a, b, c) - min(a, b, c)) <= eps and max(
        abs(cosab), abs(cosac), abs(cosbc)
    ) <= eps:
        parameters["ibrav"] = 1
        parameters["a"] = a
        cell_qe = a * np.array(
    ...
    # Note: the cell is allowed to have arbitrary rotation.
    elif (max(a, b, c) - min(a, b, c)) <= eps and max(
        abs(abs(cosab) - 1 / 2), abs(abs(cosac) - 1 / 2), abs(abs(cosbc) - 1 / 2)
    ) <= eps:
        a = a * 2**0.5
        parameters["ibrav"] = 2
        parameters["a"] = a
    ...
    # Note: the cell is allowed to have arbitrary rotation.
        ...
        abs(abs(cosab) - 1 / 3), abs(abs(cosac) - 1 / 3), abs(abs(cosbc) - 1 / 3)
    ) <= eps:
        a = a * 2 / 3**0.5
        parameters["ibrav"] = 3
        parameters["a"] = a
    ...
    # Note: the cell is allowed to have arbitrary rotation.
        ...
        abs(a - b) <= eps
        and max(abs(abs(cosab) - 1 / 2), abs(cosac), abs(cosbc)) <= eps
    ):
        parameters["ibrav"] = 4
        parameters["a"] = a
        parameters["c"] = c
    ...
    # Note: there are 6 nearest neighbours to the origin. All of them are equivalent to the origin.
    # Note: the cell is allowed to have arbitrary rotation.
    elif (max(a, b, c) - min(a, b, c)) <= eps and max(cosab, cosac, cosbc) - min(
        cosab, cosac, cosbc
    ) < eps:
        parameters["ibrav"] = 5
        parameters["a"] = a
    ...
    # Note: the cell is allowed to have arbitrary rotation.
        ...
        abs(a - b) <= eps and max(cosab, cosac, cosbc) - min(cosab, cosac, cosbc) < eps
    ):
        parameters["ibrav"] = 6
        parameters["a"] = a
        parameters["c"] = c
    ...
    elif max(a, b, c) - min(a, b, c) < eps and (
        abs(cosab - cosbc) < eps
        and abs(abs(cosab) + abs(cosbc) - cosac - 1) < eps
        or abs(cosab - cosac) < eps
        and abs(abs(cosab) + abs(cosac) - cosbc - 1) < eps
        or abs(cosbc - cosac) < eps
        and abs(abs(cosbc) + abs(cosac) - cosab - 1) < eps
    ):
    ...
    # Note: there are 4 nearest neighbours to the origin. All of them are equivalent to the origin.
    # Note: the cell is allowed to have arbitrary rotation.
    elif max(cosab, cosac, cosbc) - min(cosab, cosac, cosbc) < eps:
        parameters["ibrav"] = 8
        parameters["a"] = a
        parameters["b"] = b
        parameters["c"] = c
    ...
    # Note: the cell is allowed to have arbitrary rotation.
    elif max(abs(a - b), abs(cosac), abs(cosbc)) < eps:
        ...
        and abs(b**2 - a * b * cosab - b * c * cosbc) < eps
        and abs(c**2 - a * c * cosac - b * c * cosbc) < eps
    ):
        parameters["ibrav"] = 10
    ...
    elif max(a, b, c) - min(a, b, c) < eps and abs(cosab + cosbc + cosac + 1) < eps:
        parameters["ibrav"] = 11
        a = a * ((1 + cosbc) * 2) ** 0.5
        b = b * ((1 + cosac) * 2) ** 0.5
        c = c * ((1 + cosab) * 2) ** 0.5
    ...
    # Note: the cell is allowed to have arbitrary rotation.
    elif max(abs(cosac), abs(cosbc)) < eps:
        parameters["ibrav"] = 12
        parameters["a"] = a
        parameters["b"] = b
        parameters["c"] = c
    ...
    elif max(abs(cosab), abs(cosbc)) < eps:
        parameters["ibrav"] = -12
        parameters["a"] = a
        parameters["b"] = b
        parameters["c"] = c
...
def get_parameters_from_structure(structure, eps=1e-6):
    ...
    if parameters["ibrav"] != ibrav_ase:
        raise Warning(
            f"Found bravais lattice {parameters['ibrav']} is different from ase: {bravais_lattice_name}({ibrav_ase})"
        )
...
def get_standardized_structure_pymatgen(structure, eps=1e-6):
    ...
    spg_analyzer = SpacegroupAnalyzer(
        structure.get_pymatgen(), symprec=eps, angle_tolerance=eps
    )
...
def convert_standardized_structure_pymatgen_to_qe(pym_structure, eps=1e-6):
    ...
    if (max(a, b, c) - min(a, b, c)) <= eps and max(
        ...
        if not np.allclose(
            cell / a, np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]]), atol=eps
        ):
    ...
    # Note: the cell is allowed to have arbitrary rotation.
    elif max(abs(cosab), abs(cosac), abs(cosbc)) <= eps:
        if not np.allclose(
            cell,
            np.array(
                [
    ...
    elif abs(a - b) < eps and abs(cosac + cosbc) < eps:
        _a = ((a**2 + a * b * cosab) / 2) ** 0.5
        _b = ((a**2 - a * b * cosab) / 2) ** 0.5
        _cosac = a * cosac / _b
        _cy = c * _cosac
        ...
        if np.allclose(transformation_matrix, transformation_matrix_rounded, atol=eps):
            ...
            cell_str = np.array2string(
                cell, precision=6, separator=",", suppress_small=True
            )
            pattern_str = np.array2string(
                cell_qe, precision=6, separator=",", suppress_small=True
            )
            transformation_matrix_str = np.array2string(
                transformation_matrix_rounded,
                precision=6,
                separator=",",
                suppress_small=True,
            )
...
def check_conversion(structure, log=print):
    ...
    log(
        "Cell from StructureData",
    )
    log(np.array2string(cell, precision=6, separator=",", suppress_small=True))

    log(
        "Cell converted to Quantum ESPRESSO convention",
    )
    log(np.array2string(cell_qe, precision=6, separator=",", suppress_small=True))
    log(f"Parameters: {parameters}")
    log(f"ibrav_ase: {ibrav_ase}")
    log("Cell parameters: a, b, c, cosab, cosac, cosbc")
    log(f"                 {cellpar}")
    log("Cell parameters (QE): a, b, c, cosab, cosac, cosbc")
    log(f"                     {cellpar_qe}")
    log("-" * 100)
    ...
    log(
        "It requires a basis change to convert the cell to Quantum ESPRESSO convention",
    )
    log(np.array2string(T, precision=6, separator=",", suppress_small=True))
    ...
    log(
        np.array2string(
            reconstructed_cell_qe, precision=6, separator=",", suppress_small=True
        )
    )
    log(
        "restored cell",
    )
    log(np.array2string(restored_cell, precision=6, separator=",", suppress_small=True))
    log(
        f"Difference between reconstructed cell: {np.linalg.norm(reconstructed_cell_qe - cell_qe)}"
    )
