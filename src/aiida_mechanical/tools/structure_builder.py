...
def build_atoms_surface(
    ...
    if not isinstance(n_unit_cells, int) or n_unit_cells < 1:
        raise ValueError(
            f"Invalid number of unit cells {n_unit_cells}. Must be a positive integer."
        )

    stacking_order = n_unit_cells * "".join(layers_dict.keys())

    zs = [
        (value["z"] + cell) / n_unit_cells / (1 + vacuum_spacing)
        for cell in range(n_unit_cells)
...
def build_atoms_from_stacking_removal(
    ...
    zs = numpy.array(
        [
            value["z"] / n_unit_cells + layer / n_unit_cells
    ...
    removed_spacing = 0.0
    faulted_stacking = "".join(
        [
            char
            for i, char in enumerate(stacking_order)
            if i not in removed_layers_sorted
        ]
    )
    ...
    for removed_layer in reversed(removed_layers_sorted):
        ...
        if spacing + additional_spacing[1] < 0.0:
            raise ValueError(
                f"Spacing between removed layers is less than additional spacing: {spacing} < {additional_spacing}"
            )
    ...
    if additional_spacing[0] >= len(zs):
        raise ValueError(
            f"additional_spacing layer index {additional_spacing[0]} is out of bounds for remaining layers {len(zs)}"
        )
...
def build_atoms_from_stacking_mirror(
    ...
    connection_to_next_cell = (
        1 + layers_dict[stacking_order[0]]["z"] - layers_dict[stacking_order[-1]]["z"]
    ) * z_norm
    if print_info:
        print(spacings)
    ...
    spacings += [z - prev_z for z, prev_z in zip(zs[1:], zs[:-1])][::-1]
    ...
    spacings += [z - prev_z for z, prev_z in zip(zs[1:], zs[:-1])]
    ...
    faulted_stacking = (
        stacking_order_uc[:-1]
        + stacking_order_uc_r * n_unit_cells
        + (stacking_order_uc * (n_unit_cells - 1))[1:]
    )
    if print_info:
        print(faulted_stacking)
...
def build_atoms_from_burger_vector(
    ...
    if not isinstance(n_unit_cells, int) or n_unit_cells < 2:
        raise ValueError(
            f"Invalid number of unit cells {n_unit_cells}. Must be an integer >= 2."
        )

    zs = [
        (value["z"] + layer) / n_unit_cells / 2
        for layer in range(2 * n_unit_cells)
        for value in layers_dict.values()
    ][::-1]
...
@deprecated(
    reason="This function is not used in any workflow. Use build_atoms_from_burger_vector instead."
)
def build_atoms_from_burger_vector_with_vacuum(
    ...
    zs = [
        (value["z"] + layer) / n_unit_cells / 2 / (1 + vacuum_ratio)
        for layer in range(2 * n_unit_cells)
        for value in layers_dict.values()
    ][::-1]
