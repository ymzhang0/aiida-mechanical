...
def draw_sphere(
    ...
    # 最外层黑色边框
    rim = Circle(
        (x, y), radius, facecolor="none", edgecolor="black", linewidth=1.5, zorder=11
    )
    ax.add_patch(rim)
...
def annotate_miller(ax, v1, v2, label1, label2):
    ...
    # Calculate midpoints
    mid1 = 0.5 * np.array(v1) - [0.0, 0.2]
    mid2 = 0.5 * np.array(v2) - [0.1, 0.0]
...
def plot_layer(
    ax, pts2d, species, layer_index, a2, b2, miller_a, miller_b, alphabet=LIST_ALPHABET
):
    """
...
def get_transformed_cell(old_struct, slipping_system, unit_rep_inv):
    M = np.array(
        [
...
def plot_layers(struct, axes, slipping_system):
    ...
    if len(layers) != len(axes):
        raise ValueError(
            f"Number of layers ({len(layers)}) does not match number of axes ({len(axes)})"
        )
...
def plot_all_layers_element_colored(struct, ax, miller_a, miller_b, tol=1e-6):
    # 1) 原胞分数坐标和笛卡尔投影
    ...
    costheta = np.dot(a3, b3) / (la * lb)
    theta = np.arccos(np.clip(costheta, -1, 1))
    B2D = np.array([[la, 0.0], [lb * costheta, lb * np.sin(theta)]])
...
def group_by_plane(ase_atoms: Atoms, P, cut_plane: list[float], tol: float = 1e-6):
    grouped_by_plane = {}
    ...
    for atom in supercell:
        ...
        match = next(
            (
                k
                for k in grouped_by_plane.keys()
...
@deprecated("use Pymatgen instead")
def find_plane_pbc(
    ase_atoms: Atoms, cut_plane: list[float], max_denom: int = 10**6, tol: float = 1e-6
) -> list[list[float]]:
    """
...
@deprecated("use Pymatgen instead")
def basis_transform(v1, v2, v3, old_atoms, tol=1e-6):
    """
...
@deprecated("use Pymatgen instead")
def shortest_plane_basis(ase_atoms: Atoms, cut_plane: list[float], tol: float = 1e-6):
...
def shortest_lattice_and_plane_vectors(
    basis: numpy.ndarray, frac_coords: list[float]
) -> tuple[
    numpy.ndarray, tuple[int, int, int], list[numpy.ndarray], list[tuple[int, int, int]]
]:
    """
    计算给定晶格基矢和倒易空间坐标法向下：
    ...
    d_normal = (
        primitive_hkl_normal[0] * basis[:, 0]
    ...
    # 构造候选
    v1, v2 = ints
    candidates = {tuple(v1), tuple(v2), tuple(v1 + v2), tuple(v1 - v2), tuple(v2 - v1)}
    vecs = []
