...
def get_descendants_by_label(
    parent_workchain: orm.WorkChainNode, link_label_filter: str
) -> orm.WorkChainNode:
...
def get_descendants_by_type(
    parent_workchain: orm.WorkChainNode, link_type: LinkType = LinkType.CALL_WORK
) -> dict:
    """Get the descendant nodes of the parent workchain."""

    descendants = {}
    try:
        for node, link_type, link_label in parent_workchain.base.links.get_outgoing(
            link_type=link_type
        ).all():
...
def get_workdirs(
    ...
    workdirs = {}
    for node, link_type, link_label in workchain.base.links.get_outgoing(
        link_type=LinkType.CALL_CALC
    ).all():
        if link_label.startswith(
            "iteration_"
        ):  # only get the workdirs of the iterations
...
def check_process_state(
    ...
    source_db, source_id = process.inputs.thermo_pw.structure.base.extras.get_many(
        ("source_db", "source_id")
    )
    ...
    if process.process_state == ProcessState.WAITING:
        ...
        if process.exit_status != 0:
            ...
            if final_iteration.is_killed:
                rprint(
                    f"[bold red] {source}[{process.pk}]: {process.process_state} killed in final iteration [{final_iteration.pk}]"
                )
                return source
            if final_iteration.is_excepted:
                rprint(
                    f"[bold red] {source}[{process.pk}]: {process.process_state} excepted in final iteration [{final_iteration.pk}]"
                )
                return source
            ...
            for error_flag, error_message in (
                ...
                if error_message in stderr:
                    rprint(
                        f"[bold red] {source}[{process.pk}]: {error_flag} {error_message} in final iteration [{final_iteration.pk}]"
                    )
                    return source
            for error_flag, error_message in (
                ...
                if error_message in aiida_out:
                    ...
                    return source
            rprint(
                f"[bold red] {source}[{process.pk}]: {process.process_state} finished with exit status {process.exit_status} and final iteration [{final_iteration.pk}]"
            )
        elif not only_report_error:
            rprint(
                f"[bold green] {source}[{process.pk}]: {process.process_state} finished with exit status {process.exit_status}"
            )
    else:
