...
@deprecated(
    reason="This function is not used in any workflow. Use ASE's built-in methods instead."
)
...
def __getattr__(name):
    if name == "available_structures":
        ...
        return [
            f.stem
            for f in importlib.resources.files("aiida_mechanical.data").glob(
                "structures/cif/*.cif"
            )
        ]
...
def read_structure_from_file(formula: str, store: bool = False) -> orm.StructureData:
    ...
    if formula_str.endswith(".cif"):
        raise ValueError(
            "Please provide the chemical formula without the .cif extension (e.g. 'Al' instead of 'Al.cif')"
        )
...
def group_by_layers(
    ...
    if len(sorted_unique_z) > len(labels):
        print(
            f"Warning: Number of layers ({len(sorted_unique_z)}) exceeds number of labels ({len(labels)})."
        )
    ...
    for i, z_val in enumerate(sorted_unique_z):
        ...
        labeled_layers_dict[layer_label] = {"atoms": layer_content, "z": z_val}
...
def get_strukturbericht(
    ...
    PROTOTYPES = {
        ...
        "B1": read_structure_from_file(
            "AsTe"
        ).get_pymatgen(),  # Arsenic Telluride (AsTe)
        "B2": read_structure_from_file(
            "NiTi"
        ).get_pymatgen(),  # Arsenic Telluride (AsTe)
        "B_h": read_structure_from_file(
            "MoN"
        ).get_pymatgen(),  # Arsenic Telluride (AsTe)
        "A15": read_structure_from_file("Nb3Sn").get_pymatgen(),  # Nb3Sn (Nb3Sn)
        "C1_b": read_structure_from_file(
            "NbCoSb"
        ).get_pymatgen(),  # Gold-Copper (AuCu3)
        "L2_1": read_structure_from_file(
            "HfAlPd2"
        ).get_pymatgen(),  # Gold-Copper (AuCu3)
    ...
    try:
        ...
        for name, prototype_struct in PROTOTYPES.items():
            ...
            if matcher.fit_anonymous(struct_to_check, prototype_struct):
                if print_info:
                    print(
                        f"✅ Your structure<{atoms_to_check.get_chemical_formula()}> is of the {name} type."
                    )
                found_match = True
                return name
        ...
        print(
            "Please ensure you have a valid structure file or an API key for the Materials Project."
        )
        return None
...
def get_elements_for_wyckoff_symbols(
    ...
    return {
        wyckoff_letter: element.symbol
        for wyckoff_letter, element in zip(
            symmetrized_structure.wyckoff_letters, symmetrized_structure.elements
        )
    }
