...
FaultedStructureResult = ty.Union[
    Atoms, list[dict[str, ty.Any]], GeneralFaultStructureResult
]
...
class FaultedStructure(PlanarStructure):
    ...
    def get_faulted_structure(
        ...
        **kwargs,
        ...
        if fault_mode not in ["removal", "vacuum", "general"]:
            raise ValueError(
                f"fault_mode must be one of 'removal', 'vacuum', 'general', got '{fault_mode}'"
            )

        if fault_mode == "removal" and fault_type not in [
            "intrinsic",
            "unstable",
            "extrinsic",
        ]:
            raise ValueError(
                f"fault_type must be one of 'intrinsic', 'unstable', or 'extrinsic', got '{fault_type}'"
            )
        ...
        if (
            fault_mode == "vacuum"
            and vacuum_ratio > 0.0
            and fault_config.burger_vectors is not None
        ):
            structures_list = []
        ...
        if fault_mode == "general" and fault_config.burger_vectors is not None:
            ...
            zs = [
                (value["z"] + layer) / self.n_unit_cells
                for layer in range(self.n_unit_cells)
            ...
            if isinstance(fault_config.burger_vectors, dict):
                for direction_name, segment in fault_config.burger_vectors.items():
                    ...
                    structures_by_direction[direction_name][step_index] = (
                        self._build_general_fault_entry(
                            direction_name=direction_name,
                            step_index=step_index,
                            structure=structure,
                    ...
                    for interface, burgers_vector in segment:
                        ...
                        for _ in range(1, 1 + nsteps):
                            step_index += 1
                            current_interface_shift = interface_slips.get(
                                interface, numpy.zeros(3)
                            )
                            interface_slips[interface] = (
                                current_interface_shift + burgers_vector_step
                            )
                            faults = update_faults(
                                faults, interface, burgers_vector_step
                            )
    ...
    def _build_faulted_structure_helper(
        ...
        if config.burger_vectors is not None and isinstance(
            config.burger_vectors, list
        ):
            structures_list = []
...
class FaultedStructureData(Data):
    ...
    def _set_attribute(self, key: str, value: ty.Any) -> None:
        ...
        if self.is_stored:
            raise ModificationNotAllowed(
                "`FaultedStructureData` attributes cannot be modified after storing."
            )
    ...
    def get_structure_builder(
        self, structure: "orm.StructureData | Atoms"
    ) -> FaultedStructure:
