...
class PlanarStructure:
    ...
    def wyckoff_elements(self) -> dict[str, str]:
        ...
        return {
            w: e.symbol
            for w, e in zip(symmetrized.wyckoff_letters, symmetrized.elements)
        }
    ...
    def get_conventional_structure(
        ...
        if target_unit_vectors is not None:
            angle, axis = find_rotation(
                conventional_structure.cell.T, numpy.array(target_unit_vectors).T
            )
...
class CleavagedStructureData(Data):
    ...
    def __init__(
        ...
        **kwargs: ty.Any,
        ...
        resolved_vacuum_spacings = [
            float(value) for value in (vacuum_spacings or [1.0])
        ]
    ...
    def _set_attribute(self, key: str, value: ty.Any) -> None:
        ...
        if self.is_stored:
            raise ModificationNotAllowed(
                "`CleavagedStructureData` attributes cannot be modified after storing."
            )
    ...
    def vacuum_spacings(self) -> list[float]:
        ...
        return [
            float(value)
            for value in self.base.attributes.get(self.VACUUM_SPACINGS_KEY, [1.0])
        ]
