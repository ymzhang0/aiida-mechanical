...
class StructureGenerationMixin:
    ...
    def _calculate_structure_multiplier(
    ...
    def _store_structure_multiplier(
        self, structure: orm.StructureData | Atoms, multiplier_name: str
    ) -> int:
    ...
    def _validate_faulted_structure(
        ...
        if faulted_structure_data is None:
            self.report(
                f"{fault_type.capitalize()} fault structure is not available for this gliding system."
            )
        ...
        if actual_structure is None:
            self.report(
                f"{fault_type.capitalize()} fault structure is missing structure data."
            )
            return False, self.exit_codes.ERROR_NO_STRUCTURE_TYPE_DETECTED
...
class EnergyCalculationMixin:
    ...
    def _get_physical_constant(self, name: str) -> float:
        ...
        if not hasattr(self, name):
            raise AttributeError(
                f"`{type(self).__name__}` must define `{name}` to use energy mixin helpers."
            )
        return getattr(self, name)

    def _calculate_stacking_fault_energy(
        self, total_energy_faulted: float, fault_multiplier: int, fault_type_name: str
    ) -> ty.Optional[float]:
        ...
        stacking_fault_energy = (
            energy_difference
            / self.ctx.surface_area
            * self._get_physical_constant("_eVA22Jm2")
        )
    ...
    def _calculate_surface_energy(
        self, total_energy_slab: float, surface_multiplier: int
    ) -> ty.Optional[float]:
...
class KpointsSetupMixin:
    ...
    def _calculate_kpoints_for_structure(
        ...
        conventional_ase = StructureGenerationMixin._ensure_ase_structure(
    ...
    def _setup_surface_energy_kpoints(
        ...
        return self._calculate_kpoints_for_structure(
            self.ctx.cleavaged_structure, kpoints_scf
        )
...
def clean_workchain_calcs(workchain: orm.WorkChainNode) -> list[int]:
    ...
    for called_descendant in workchain.called_descendants:
        if isinstance(called_descendant, orm.CalcJobNode) and clean_calcjob_remote(
            called_descendant
        ):
