...
class GSFERelaxWorkChain(
    ...
    def generate_structures(self) -> ty.Optional[ExitCode]:
        ...
        for output_label, output_node in generated_structures.items():
            if output_label in (
                ...
                "surface_area",
            ) or not isinstance(output_node, orm.StructureData):
                continue
            ...
            if None in (
                ...
                self.report(
                    f"Incomplete faulted-structure entry generated for `{output_label}`."
                )
                return self.exit_codes.ERROR_NO_STRUCTURE_TYPE_DETECTED
        ...
        if not self.ctx.generated_structures:
            self.report(
                "No generalized fault path is available for the selected structure and gliding plane."
            )
            return self.exit_codes.ERROR_NO_STRUCTURE_TYPE_DETECTED
    ...
    def should_run_sfe(self) -> bool:
        ...
        self.ctx.current_multiplier = self._calculate_structure_multiplier(
            self.ctx.current_structure
        )
    ...
    def run_sfe(self) -> dict[str, ty.Any]:
        inputs = AttributeDict(
            self.exposed_inputs(PwRelaxWorkChain, namespace=self._SFE_NAMESPACE)
        )
        ...
        return {"workchain_sfe": append_(running)}
        ...
        self.ctx.sfe_results.append(
            {
                ...
                "burger_vector": [
                    float(value) for value in self.ctx.current_burger_vector
                ],
                "total_cell_shift": [
                    float(value) for value in self.ctx.current_total_cell_shift
                ],
                "interface_slips": {
                    str(interface): [float(value) for value in interface_shift]
    ...
    def results(self) -> None:
        ...
        for point_result in self.ctx.sfe_results:
            direction_results = nested_results.setdefault(
                point_result["direction_name"], {}
            )
        ...
        if "total_energy_conventional_geometry" in self.ctx:
            results["conventional_energy_ev"] = float(
