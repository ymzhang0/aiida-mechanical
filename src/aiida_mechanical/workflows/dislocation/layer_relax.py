...
class RigidLayerRelaxWorkChain(ProtocolMixin, WorkChain):
    ...
    def should_run_relax(self):
        ...
        fault_method = (
            self.inputs.fault_method.value.lower()
            if self.inputs.fault_method.value
            else "removal"
        )
        gliding_plane = (
        ...
        # Validate structure
        if faulted_structure_data is None or not faulted_structure_data.get(
            "structures"
        ):
            self.report(
                f"Faulted structure not available for spacing {current_spacing}. Skipping."
            )
            return False
        ...
        if actual_structure is None:
            self.report(
                f"Faulted structure is missing for spacing {current_spacing}. Skipping."
            )
            return False
    ...
    def setup_supercell_kpoints(self):
        ...
        z_ratio = (
            self.ctx.current_structure.get_ase().cell.cellpar()[2]
            / self.inputs.structure.get_ase().cell.cellpar()[2]
        )
        kpoints_mesh = self.inputs.kpoints.get_kpoints_mesh()[0]

        kpoints_relax = orm.KpointsData()
        kpoints_relax.set_kpoints_mesh(
            kpoints_mesh[:2] + [ceil(kpoints_mesh[2] / z_ratio)]
        )

        self.ctx.kpoints_relax = kpoints_relax
        self.report(
            f"Kpoints mesh for rigid layer relaxation (spacing {self.ctx.current_spacing}): {kpoints_relax.get_kpoints_mesh()[0]}"
        )
    ...
    def run_relax(self):
        ...
        self.report(
            f"launching PwRelaxWorkChain<{running.pk}> for spacing: {self.ctx.current_spacing}."
        )
    ...
    def inspect_relax(self):
        ...
        self.report(
            f"PwRelaxWorkChain<{workchain.pk}> for spacing {self.ctx.current_spacing} finished successfully."
        )
