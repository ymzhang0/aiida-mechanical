...
class SFEBaseWorkChain(
    ...
    _RYA22Jm2 = 4.3597447222071e-18 / 2 * 1e20
    ...
    def define(cls, spec):
        ...
        spec.input(
            ...
            valid_type=orm.List,
            required=False,
            default=lambda: orm.List(list=[0.0]),
    ...
    def get_builder_from_protocol(
        cls, code, structure, protocol="moderate", overrides=None, **kwargs
    ):
        ...
        for namespace, workchain_type in [
            ...
            if workchain_type == RigidLayerRelaxWorkChain:
                overrides.setdefault("relax", {}).setdefault("base_relax", {})[
                    "pseudo_family"
                ] = inputs.get("pseudo_family", None)
                overrides.setdefault("relax", {}).setdefault("base_init_relax", {})[
                    "pseudo_family"
                ] = inputs.get("pseudo_family", None)
    ...
    def run_relax(self):
        ...
        self.report(
            f"launching PwRelaxWorkChain<{running.pk}> for {self.inputs.structure.get_formula()} unit cell geometry."
        )
    ...
    def inspect_relax(self):
        ...
        self.report(
            f"PwRelaxWorkChain<{workchain.pk}> for {self.inputs.structure.get_formula()} unit cell geometry finished OK"
        )
        ...
        self.ctx.total_energy_unit_cell = workchain.outputs.output_parameters.get(
            "energy"
        )
        self.report(
            f"Total energy of unit cell after relaxation: {self.ctx.total_energy_unit_cell / self._RY2eV} Ry"
        )
    ...
    def generate_structures(self):
        ...
        if strukturbericht:
            ...
            self.report("Strukturbericht can not be detected.")
        ...
        unit_cell_formula = Formula(
            self.ctx.current_structure.get_ase().get_chemical_formula()
        )
    ...
    def _get_kpoints_scf(self):
        ...
        if "kpoints_scf" in self.ctx:
            ...
            inputs = {
                "structure": orm.StructureData(ase=self.ctx.conventional_structure),
    ...
    def setup(self):
        ...
        self.ctx.kpoints_surface_energy = self._setup_surface_energy_kpoints(
            kpoints_scf
        )
    ...
    def run_scf(self):
        ...
        inputs.pw.structure = orm.StructureData(ase=self.ctx.conventional_structure)

        inputs.kpoints_distance = self.inputs.kpoints_distance

        running = self.submit(PwBaseWorkChain, **inputs)
        self.report(
            f"launching PwBaseWorkChain<{running.pk}> for {self.inputs.structure.get_formula()} conventional geometry."
        )
    ...
    def run_layer_relax(self):
        inputs = AttributeDict(
            self.exposed_inputs(
                RigidLayerRelaxWorkChain, namespace=self._RIGID_LAYER_RELAX_NAMESPACE
            )
        )
        inputs.structure = orm.StructureData(ase=self.ctx.conventional_structure)
        ...
        self.report(
            f"launching RigidLayerRelaxWorkChain<{running.pk}> for rigid layer relaxation calculations over all spacings."
        )

        return {"workchain_layer_relax": running}
