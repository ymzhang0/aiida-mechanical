...
class Thermo_pwParser(BaseParser):
    ...
    _OUTPUT_ELASTIC_CONSTANTS_SUBFOLDER = Thermo_pwCalculation._OUTPUT_ELASTIC_CONSTANTS_SUBFOLDER
    _OUTPUT_GNUPLOT_FILES_SUBFOLDER = Thermo_pwCalculation._OUTPUT_GNUPLOT_FILES_SUBFOLDER
    ...
    def parse(self, **kwargs):
        ...
        # --- elastic constants ---
        if self._OUTPUT_ELASTIC_CONSTANTS_FILE in self.retrieved.base.repository.list_object_names():
            elastic_constants_contents = self.retrieved.base.repository.get_object_content(self._OUTPUT_ELASTIC_CONSTANTS_FILE)
            self.out('elastic_constants', self.parse_elastic_constants(elastic_constants_contents))

        # --- thermal properties ---
        if self._OUTPUT_THERM_DEBYE_FILE in self.retrieved.base.repository.list_object_names():
            therm_dat_debye_contents = self.retrieved.base.repository.get_object_content(self._OUTPUT_THERM_DEBYE_FILE)
            self.out('therm_dat_debye', self.parse_therm_dat_debye(therm_dat_debye_contents))
    ...
    def parse_stdout(stdout, logs):
        ...
        data_type_regex = [
            ...
            ('total_scf_calculations', int, re.compile(r'for a total of\s+(\d+)\s+scf calculations')),
            ...
            ('approx_debye_temp', float, re.compile(r"The approximate Debye temperature is\s*" + FLOAT_NUM)),
            ('avg_debye_velocity', float, re.compile(r"Average Debye sound velocity\s*=\s*" + FLOAT_NUM)),
        ...
        def parse_elastic_constants_type(stdout):
            ...
            if match:
                ...
                elastic_constants_type = [line.split()[1:-1] for line in elastic_tensor_raw.split('\n')[:6]]
        ...
        def parse_elastic_constants_fitting(stdout):
            ...
            for match in block_pattern.finditer(stdout):
                ...
                strains = []
                stresses = []
                strain_stress_pairs = re.findall(r'(-?\d+\.\d+)\s+([-\d\.E+]+)', table_raw_string)
        ...
        def parse_moduli(stdout):
            ...
            definitions = {
                'voigt': {
                    'block_regex': re.compile(r"Voigt approximation:\s*([\s\S]*?)(?=\n\s*Reuss approximation:|\Z)"),
                    'target_dict': results['moduli'],
                    'properties': {
                        'bulk_modulus_B': re.compile(r"Bulk modulus\s+B\s*=\s*(-?[\d\.]+)"),
                        'young_modulus_E': re.compile(r"Young modulus\s+E\s*=\s*(-?[\d\.]+)"),
                        'shear_modulus_G': re.compile(r"Shear modulus\s+G\s*=\s*(-?[\d\.]+)"),
                        'poisson_ratio_n': re.compile(r"Poisson Ratio\s+n\s*=\s*(-?[\d\.]+)"),
                        'pugh_ratio_r': re.compile(r"Pugh Ratio\s+r\s*=\s*(-?[\d\.]+)")
                    }
                },
                'reuss': {
                    'block_regex': re.compile(r"Reuss approximation:\s*([\s\S]*?)(?=\n\s*Voigt-Reuss-Hill average|\Z)"),
                    'target_dict': results['moduli'],
                    'properties': {
                        'bulk_modulus_B': re.compile(r"Bulk modulus\s+B\s*=\s*(-?[\d\.]+)"),
                        'young_modulus_E': re.compile(r"Young modulus\s+E\s*=\s*(-?[\d\.]+)"),
                ...
                'vrh': {
                    'block_regex': re.compile(r"Voigt-Reuss-Hill average of the two approximations:\s*([\s\S]*?)(?=\n\s*Voigt-Reuss-Hill average; sound velocities:|\Z)"),
                    'target_dict': results['moduli'],
                    'properties': {
                        'bulk_modulus_B': re.compile(r"Bulk modulus\s+B\s*=\s*(-?[\d\.]+)"),
                        'young_modulus_E': re.compile(r"Young modulus\s+E\s*=\s*(-?[\d\.]+)"),
                        'shear_modulus_G': re.compile(r"Shear modulus\s+G\s*=\s*(-?[\d\.]+)"),
                        'longitudinal_modulus_L': re.compile(r"Longitudinal modulus\s+L\s*=\s*(-?[\d\.]+)"),
                        'poisson_ratio_n': re.compile(r"Poisson Ratio\s+n\s*=\s*(-?[\d\.]+)"),
                        'pugh_ratio_r': re.compile(r"Pugh Ratio\s+r\s*=\s*(-?[\d\.]+)")
                    }
                },
                'velocities': {
                    'block_regex': re.compile(r"Voigt-Reuss-Hill average; sound velocities:\s*([\s\S]*?)(?=\n\s*\n|\Z)"),
                    ...
                    'properties': {
                        'compressional_V_P': re.compile(r"Compressional\s+V_P\s*=\s*(-?[\d\.]+)"),
    ...
    def parse_elastic_constants(content):
        ...
        elastic_constants = numpy.array(matrix_blocks[0].split(), dtype=float).reshape((6, 6))
        elastic_compliances = numpy.array(matrix_blocks[1].split(), dtype=float).reshape((6, 6))
