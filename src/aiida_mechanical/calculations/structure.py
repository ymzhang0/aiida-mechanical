    ...
    if isinstance(generated, list):
        ...
        for index, item in enumerate(generated):
            ...
            if not isinstance(item, dict) or "structure" not in item:
                raise TypeError(
                    "Unsupported faulted structure payload returned by `FaultedStructureData`."
                )
            ...
            normalized.append(
                {
                    "label": str(
                        item.get(
                            "label",
                            f"sfe_{item.get('direction_name', fault_type)}_{index:03d}",
                        )
                    ),
                    "structure": item["structure"],
                    "burger_vector": [
                        float(value) for value in item.get("burger_vector", [])
                    ],
                    "total_cell_shift": [
                        float(value)
                        for value in item.get(
                            "total_cell_shift", item.get("burger_vector", [])
                        )
                    ],
                    "interface_slips": {
                        int(interface): [float(value) for value in interface_shift]
                        for interface, interface_shift in item.get(
                            "interface_slips", {}
                        ).items()
                    },
                ...
                normalized.append(
                    {
                        ...
                        "burger_vector": [
                            float(value) for value in metadata["burger_vector"]
                        ],
                        "total_cell_shift": [
                            float(value) for value in metadata["total_cell_shift"]
                        ],
                        "interface_slips": {
                            int(interface): [float(value) for value in interface_shift]
                            for interface, interface_shift in metadata[
                                "interface_slips"
                            ].items()
                        },
...
def generate_faulted_structures(
    ...
    if not normalized_points:
        raise ValueError(
            "No faulted structures could be generated for the requested configuration."
        )

    outputs: dict[str, orm.Data] = {
        "conventional_structure": orm.StructureData(
            ase=builder.get_conventional_structure()
        ),
        ...
        structure_node.base.extras.set_many(
            {
                ...
                "total_cell_shift": [
                    float(value) for value in point["total_cell_shift"]
                ],
                "interface_slips": {
...
def generate_cleavaged_structures(
    ...
    if not vacuum_spacings:
        raise ValueError(
            "No vacuum spacings configured for cleavaged structure generation."
        )
    ...
    for vacuum_spacing in vacuum_spacings:
        ...
        if slab_key in outputs or spacing_output_key in outputs:
            raise ValueError(
                f"Duplicate vacuum spacing key generated for {vacuum_spacing}."
            )
